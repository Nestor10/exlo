  ...
  private def stage(emissions: ZStream[Any, ExloError, Emission[String, Long]]): Stage[Unit, String, Long, Any] =
    new Stage[Unit, String, Long, Any]:
      ...
      def run[R0](
          ...
          resume: Long
      ): ZStream[Any & R0, ExloError, Emission[String, Long]] = emissions
  ...
  def spec = suite("Exlo.run")(
    test("end-to-end: records reach the sink, state commits, RunContext is set") {
      ...
      for
        ...
        _        <- Exlo.run("smoke-connector", stage(captured), fastFlush)
                      .provide(
        ...
        state    <- storeRef.readByKey("smoke-connector", "smoke-stream", StateStore.WatermarkKey)
        ctx      <- observed.get
      yield assertTrue(
        rows.map(_.value).toList == List("""{"a":1}""", """{"a":2}"""),
        state.exists(_.value == "7"),
        ctx._2 == "smoke-connector",
        ctx._3 == "1.0.0",
        ctx._4 == "smoke-stream",
