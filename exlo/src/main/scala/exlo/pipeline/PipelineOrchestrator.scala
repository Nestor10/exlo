 ...
 *
 * Pulls namespace and tableName from StreamConfig to keep coupling low.
 */
object PipelineOrchestrator:
   ...
   *
   * Pulls namespace and tableName from config.
   *
   * @param syncMetadata
  ...
  def run(
    ...
    for
      ...
      _            <- ZIO.unless(tableExists) {
        ZIO.logInfo(s"Table ${streamConfig.namespace}.${streamConfig.tableName} does not exist, creating...") *>
          catalog.createTable(streamConfig.namespace, streamConfig.tableName, customLocation = None)
      }
